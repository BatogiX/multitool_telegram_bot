...
async def enter_services(query: CallbackQuery, state: FSMContext, callback_data: PwdMgrCb.EnterServices) -> Message:
    ...
    await StorageUtils.set_pm_services_offset(offset=services_offset, state=state)
    services = await db_manager.relational_db.get_services(query.from_user.id, services_offset)
...
async def create_service(query: CallbackQuery, state: FSMContext, callback_data: PwdMgrCb.CreateService) -> Message:
    ...
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_pm_input_format_text(CREATE_SERVICE_TEXT, state)
...
async def delete_services(query: CallbackQuery, state: FSMContext, callback_data: PwdMgrCb.DeleteServices) -> Message:
    ...
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_pm_input_format_text(DELETE_SERVICES_TEXT, state)

    return await query.message.edit_text(
        text=DELETE_SERVICES_TEXT,
        reply_markup=Keyboards.inline.return_to_services(services_offset=offset)
    )
...
async def enter_service(query: CallbackQuery, callback_data: PwdMgrCb.EnterService, state: FSMContext) -> Optional[Message]:
    ...
    await StorageUtils.set_pm_services_offset(services_offset, state)
    await StorageUtils.set_pm_pwd_offset(pwd_offset, state)
    await StorageUtils.set_pm_input_format_text(ASK_MASTER_PASSWORD_TEXT, state)
    ...
    if query.message:
        ...
        return await query.message.edit_text(
            ...
            reply_markup=Keyboards.inline.return_to_services(services_offset=services_offset)
        )
...
async def create_password(query: CallbackQuery, callback_data: PwdMgrCb.CreatePassword, state: FSMContext) -> Message:
    ...
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_service(service, state)
    await StorageUtils.set_pm_input_format_text(CREATE_PASSWORD_TEXT, state)
...
async def enter_password(query: CallbackQuery, callback_data: PwdMgrCb.EnterPassword, state: FSMContext) -> Message:
    ...
    service = BotUtils.escape_markdown_v2(await StorageUtils.get_service(state))
...
async def change_service(query: CallbackQuery, state: FSMContext, callback_data: PwdMgrCb.ChangeService) -> Message:
    ...
    await StorageUtils.set_service(old_service, state)
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_pm_input_format_text(CHANGE_SERVICE_TEXT, state)
...
async def delete_service(query: CallbackQuery, state: FSMContext, callback_data: PwdMgrCb.DeleteService) -> Message:
    ...
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_service(service_to_delete, state)
    await StorageUtils.set_pm_input_format_text(DELETE_SERVICE_TEXT, state)
...
async def delete_password(query: CallbackQuery, state: FSMContext) -> Message:
    ...
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_pm_input_format_text(DELETE_PASSWORD_TEXT, state)
...
async def import_from_file(query: CallbackQuery, state: FSMContext) -> Message:
    ...
    await StorageUtils.set_message_id_to_delete(query.message.message_id, state)
    await StorageUtils.set_pm_input_format_text(IMPORT_FROM_FILE_TEXT, state)
