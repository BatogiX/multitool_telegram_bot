...
import helpers.pwd_mgr as helper
import keyboards.inline
from database import db
from helpers.pwd_mgr import DecryptedPassword, EncryptedPassword, texts
...
async def create_service(message: Message, state: FSMContext) -> Message:
    ...
    await db.nosql.set_service(service, state)

    decrypted_record = DecryptedPassword(service=service, login=login, password=password)
    ...
    return await message.answer(
        ...
        reply_markup=keyboards.inline.pwd_mgr_passwords_ikm(
            ...
            service=service,
            pwd_offset=0,
            services_offset=await db.nosql.get_services_offset(state),
        ),
    )
...
async def create_password(message: Message, state: FSMContext) -> Message:
    ...
    service = await db.nosql.get_service(state)
...
async def delete_password(message: Message, state: FSMContext) -> Message:
    ...
    service, pwds_offset, services_offset = await db.nosql.execute(
        db.nosql.get_service(state),
        db.nosql.get_pwds_offset(state),
        db.nosql.get_services_offset(state),
    )
    encrypted_records = await db.sql.get_passwords_by_service(
        user_id=message.from_user.id,
        service=service,
        offset=pwds_offset,
    )

    decrypted_records = await DecryptedPassword.decrypt(
    ...
    if decrypted_records:
        return await message.answer(
            text=texts.PASSW_DELETED_TEXT + texts.gen_passwords_text(service, pwds_offset),
            parse_mode="Markdown",
            reply_markup=keyboards.inline.pwd_mgr_passwords_ikm(
                decrypted_records,
                service,
    ...
    services = await db.sql.get_services(message.from_user.id, offset=0)
    if services:
        return await message.answer(
            text=texts.PASSW_DELETED_TEXT + texts.gen_services_text(services_offset=0),
            reply_markup=keyboards.inline.pwd_mgr_services_ikm(services, services_offset=0),
        )
    return await message.answer(
        text=texts.PASSW_DELETED_TEXT + texts.NO_SERVICES_TEXT,
...
async def update_credentials(message: Message, state: FSMContext) -> Message:
    ...
    decrypted_records = await DecryptedPassword.decrypt(encrypted_records, derived_key)
    for i, decrypted_record in enumerate(decrypted_records):
        if decrypted_record.login == login and decrypted_record.password == password:
            new_pwd_record = await EncryptedPassword.encrypt(
                [
                    DecryptedPassword(
                        service=service,
                        login=new_login,
            ...
            await db.sql.update_credentials(
                user_id=message.from_user.id,
                service=service,
...
async def change_service(message: Message, state: FSMContext) -> Message:
    ...
    old_service, services_offset, pwds_offset = await db.nosql.execute(
        db.nosql.get_service(state),
        db.nosql.get_services_offset(state),
        db.nosql.get_pwds_offset(state),
    )
    await db.sql.change_service(
    ...
    decrypted_records = await DecryptedPassword.decrypt(
        await db.sql.get_passwords_by_service(message.from_user.id, new_service, offset=0),
        derived_key,
    )
    return await message.answer(
...
async def delete_services(message: Message, state: FSMContext) -> Message:
    ...
    await db.sql.delete_passwords(message.from_user.id)
    return await message.answer(
...
async def delete_service(message: Message, state: FSMContext) -> Message:
    ...
    coroutines = [
        db.sql.get_services(message.from_user.id, offset=0),
        db.sql.delete_passwords_by_service(message.from_user.id, service),
    ]
...
async def import_from_file(message: Message, state: FSMContext) -> Message:
    ...
    try:
        ...
        record = await db.sql.get_rand_password(message.from_user.id)
        return await message.answer(
            text=f"{e!s}\n\n{texts.ENTER_TEXT}",
...
async def change_master_password(message: Message, state: FSMContext) -> Message:
    ...
    encrypted_records = await db.sql.export_passwords(message.from_user.id)
    decrypted_records = await DecryptedPassword.decrypt(encrypted_records, old_key)
    encrypted_records = await EncryptedPassword.encrypt(decrypted_records, new_key)
    await db.sql.delete_passwords(message.from_user.id)
    await db.sql.import_passwords(message.from_user.id, encrypted_records)
