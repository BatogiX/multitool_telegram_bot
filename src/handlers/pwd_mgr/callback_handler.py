...
from helpers.pwd_mgr import texts
...
async def enter(query: CallbackQuery) -> None:
    record = await db.sql.get_rand_password(query.from_user.id)
...
async def enter_services(
    query: CallbackQuery,
    state: FSMContext,
    callback_data: PwdMgrCb.EnterServices,
) -> Message:
    services_offset = callback_data.services_offset

    services, _ = await asyncio.gather(
        db.sql.get_services(query.from_user.id, services_offset),
        db.nosql.execute(
            db.nosql.set_services_offset(services_offset, state),
...
async def create_service(
    query: CallbackQuery,
    state: FSMContext,
    callback_data: PwdMgrCb.CreateService,
) -> Message:
    record, _ = await asyncio.gather(
        db.sql.get_rand_password(query.from_user.id),
        db.nosql.execute(
            db.nosql.set_message_id_to_delete(query.message.message_id, state),
            db.nosql.set_input_format_text(texts.CREATE_SERVICE_TEXT, state),
            db.nosql.set_state(PasswordManagerStates.CreateService.state, state),
        ),
    )
...
async def delete_services(
    query: CallbackQuery,
    state: FSMContext,
    callback_data: PwdMgrCb.DeleteServices,
) -> Message:
    await db.nosql.execute(
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.DELETE_SERVICES_TEXT, state),
        db.nosql.set_state(PasswordManagerStates.DeleteService.state, state),
    )
...
async def enter_service(
    ...
    coroutines = [
        db.nosql.set_service(callback_data.service, state),
        db.nosql.set_pwds_offset(callback_data.pwds_offset, state),
        db.nosql.set_input_format_text(texts.ASK_MASTER_PASSW_TEXT, state),
        db.nosql.set_state(PasswordManagerStates.EnterService.state, state),
    ]
    if query.message:
        coroutines.append(db.nosql.set_message_id_to_delete(query.message.message_id, state))
    ...
    if not query.message:
        await query.bot.send_message(
            ...
            text=texts.ASK_MASTER_PASSW_TEXT,
            reply_markup=keyboards.inline.return_to_services_ikm(callback_data.services_offset),
        )
        return None
...
async def create_password(
    ...
    await db.nosql.execute(
        db.nosql.set_service(callback_data.service, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.CREATE_PASSW_TEXT, state),
        db.nosql.set_state(PasswordManagerStates.CreatePassword.state, state),
    )

    return await query.message.edit_text(
        text=texts.CREATE_PASSW_TEXT,
        reply_markup=keyboards.inline.return_to_passwords_ikm(
            callback_data.service,
            callback_data.services_offset,
            callback_data.pwds_offset,
        ),
    )
...
async def enter_password(
    query: CallbackQuery,
    callback_data: PwdMgrCb.EnterPassword,
    state: FSMContext,
) -> Message:
    login = escape_markdown_v2(callback_data.login)
    password = escape_markdown_v2(callback_data.password)

    service, services_offset, pwds_offset = await db.nosql.execute(
        db.nosql.get_service(state),
        db.nosql.get_services_offset(state),
        db.nosql.get_pwds_offset(state),
    )
...
async def change_service(
    ...
    await db.nosql.execute(
        db.nosql.set_service(callback_data.service, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.CHANGE_SERVICE_TEXT, state),
        db.nosql.set_state(PasswordManagerStates.ChangeService.state, state),
        db.nosql.set_pwds_offset(callback_data.pwds_offset, state),
    )
...
async def delete_service(
    ...
    await db.nosql.execute(
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_service(callback_data.service, state),
        db.nosql.set_input_format_text(texts.DELETE_SERVICE_TEXT, state),
        db.nosql.set_state(PasswordManagerStates.DeleteService.state, state),
    )
...
async def delete_password(
    query: CallbackQuery,
    state: FSMContext,
    callback_data: PwdMgrCb.DeletePassword,
) -> Message:
    login, password = callback_data.login, callback_data.password  # No need to escape MarkdownV2

    service, _, _, _ = await db.nosql.execute(
        db.nosql.get_service(state),
        db.nosql.set_state(PasswordManagerStates.DeletePassword.state, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.DELETE_PASSW_TEXT, state),
    )

    credentials = texts.gen_credentials(service, login, password)
    text = escape_markdown_v2(texts.DELETE_PASSW_TEXT)
    return await query.message.edit_text(
...
async def update_credentials(
    query: CallbackQuery,
    state: FSMContext,
    callback_data: PwdMgrCb.UpdateCredentials,
) -> Message:
    login, password = callback_data.login, callback_data.password  # No need to escape MarkdownV2

    service, _, _, _ = await db.nosql.execute(
        db.nosql.get_service(state),
        db.nosql.set_state(PasswordManagerStates.UpdateCredentials.state, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.UPDATE_CREDENTIALS_TEXT, state),
    )
...
async def import_from_file(query: CallbackQuery, state: FSMContext) -> Message:
    await db.nosql.execute(
        db.nosql.set_state(PasswordManagerStates.ImportFromFile.state, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.IMPORT_FROM_FILE_TEXT, state),
    )
...
async def export_to_file(query: CallbackQuery, state: FSMContext) -> Message:
    await db.nosql.execute(
        db.nosql.set_state(PasswordManagerStates.ExportToFile.state, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.ASK_MASTER_PASSW_TEXT, state),
    )
...
async def change_master_password(query: CallbackQuery, state: FSMContext) -> Message:
    await db.nosql.execute(
        db.nosql.set_state(PasswordManagerStates.ChangeMasterPassword.state, state),
        db.nosql.set_message_id_to_delete(query.message.message_id, state),
        db.nosql.set_input_format_text(texts.CHANGE_MASTER_PASSW_TEXT, state),
    )

    return await query.message.edit_text(
        text=texts.CHANGE_MASTER_PASSW_TEXT,
        reply_markup=keyboards.inline.RETURN_TO_PWD_MGR_IKM,
    )
