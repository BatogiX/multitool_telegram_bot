...
from utils import delete_file, delete_fsm_message, download_file
...
MSG_ERROR_MASTER_PASSW = "Wrong Master Password"
...
async def show_service_logins(
    ...
    derived_key: bytes,
    service: str,
) -> tuple[list[DecryptedPassword], int, int]:
    pwd_offset, services_offset = await db.nosql.execute_batch(
        db.nosql.get_pwds_offset(state),
        db.nosql.get_services_offset(state),
    )
    encrypted_records = await db.sql.get_passwords(
...
async def create_password_record(
    decrypted_record: DecryptedPassword,
    user_id: int,
    derived_key: bytes,
) -> None:
    encrypted_record = await Password.encrypt(decrypted_record, derived_key)
    await db.sql.add_password(
        user_id,
        encrypted_record.service,
...
async def resend_user_input_request(
    ...
    input_format, _ = await db.nosql.execute_batch(
        db.nosql.get_input_format_text(state),
        db.nosql.set_state(current_state, state),
    )
    message_to_delete = await message.answer(
        text=f"{error_message}\n\n{input_format}",
        reply_markup=keyboards.inline.return_to_services_ikm(services_offset=0),
    )
    await db.nosql.set_message_id_to_delete(message_to_delete.message_id, state)
    return message_to_delete


async def handle_message_deletion(state: FSMContext, message: Message) -> str:
    results, _ = await asyncio.gather(
        db.nosql.execute_batch(
            db.nosql.get_state(state),
...
async def process_importing_from_file(message: Message, derived_key: bytes):
    ...
    encrypted_records = await Password.encrypt(decrypted_records, derived_key)

    await db.sql.import_passwords(
        user_id=message.from_user.id,
...
async def process_exporting_to_file(derived_key: bytes, user_id: int) -> BufferedInputFile:
    encrypted_records = await db.sql.export_passwords(user_id=user_id)
    decrypted_records = await DecryptedPassword.decrypt(encrypted_records, derived_key)
...
async def validate_master_password(master_password: str, user_id: int) -> bytes:
    ...
    salt = await db.sql.get_salt(user_id)
    return await derive_key(master_password, salt)


async def validate_derived_key(user_id: int, derived_key: bytes) -> None:
    rand_encrypted_record = await db.sql.get_rand_password(user_id)
    if rand_encrypted_record:
        try:
            await DecryptedPassword.decrypt(rand_encrypted_record, derived_key)
        except InvalidTag:
            raise InvalidTag(MSG_ERROR_MASTER_PASSW)
