...
from utils import delete_file, download_file

DEFAULT_CHUNK_SIZE = 65536

_HASH_TO_STATE_MAPPING: dict[str, State] = {
    HashMenuCallbackData.HashTypes.MD5: HashMenuStates.MD5,
    HashMenuCallbackData.HashTypes.SHA1: HashMenuStates.SHA1,
    HashMenuCallbackData.HashTypes.SHA256: HashMenuStates.SHA256,
}
_HASH_TO_FUNC_MAPPING: dict[str, Callable[[Any], HASH]] = {
    HashMenuCallbackData.HashTypes.MD5: hashlib.md5,
    HashMenuCallbackData.HashTypes.SHA1: hashlib.sha1,
    HashMenuCallbackData.HashTypes.SHA256: hashlib.sha256,
}
...
async def check_hash(state: FSMContext, message: types.Message) -> tuple[bool, str, str, str]:
    hash_type = await db.nosql.get_hash_type(state)
